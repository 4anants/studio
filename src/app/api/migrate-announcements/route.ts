...
export async function GET() {
    try {
        console.log('Starting announcements table cleanup...');
        ...
        try {
            ...
            `);
            console.log('✓ Removed expires_on column');
        } catch (error: any) {
            if (error.code === 'ER_CANT_DROP_FIELD_OR_KEY') {
                console.log('- expires_on column does not exist');
            } else {
                ...
                console.log(`- could not remove expires_on: ${error.message}`);
            }
        }
